    ...
    return (
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <button className={cn("h-7 min-w-7 shrink-0 flex flex-col items-center justify-center rounded-sm hover:bg-neutral-200/80 px-1.5 text-sm overflow-hidden")}>
...
const FontSizeButton = () => {
    ...
    return (
        <div className="flex items-center gap-x-0.5">
            <button onClick={decrement} className={cn("h-7 w-7 shrink-0 flex  items-center justify-center rounded-sm hover:bg-neutral-200/80")}>
                <MinusIcon className="size-4" />
            </button>
            {isEditing ? (
                <input type='text' value={inputValue} onChange={handleInputChange} onBlur={handleInputBlur} onKeyDown={handleKeyDown}
                    className={cn("h-7 w-10 text-sm text-center border border-neutral-400 rounded-sm bg-transparent focus:outline-none focus:ring-0")}
                />
            ) : (
                <button onClick={() => {
                    setIsEditing(true);
                    setFontSize(currentFontSize)
                }} className={cn("h-7 w-10 text-sm text-center border border-neutral-400 rounded-sm bg-transparent cursor-text")}>
                    {currentFontSize}
                </button>
            )}
            <button onClick={increment} className={cn("h-7 w-7 shrink-0 flex  items-center justify-center rounded-sm hover:bg-neutral-200/80")}>
...
const LinkButton = () => {
    ...
    return (<DropdownMenu onOpenChange={(open) => { if (open) setValue(editor?.getAttributes('link').href || '') }}>
        <DropdownMenuTrigger asChild>
            <button className={cn("h-7 min-w-7 shrink-0 flex flex-col items-center justify-center rounded-sm hover:bg-neutral-200/80 px-1.5 text-sm overflow-hidden")}>
...
const HeadingLevelButton = () => {
    ...
    return (
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <button className={cn("h-7 min-w-7 shrink-0 flex items-center justify-center rounded-sm hover:bg-neutral-200/80 px-1.5 text-sm overflow-hidden")}>
...
const FontFamilyButton = () => {
    ...
    return (
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
                <button className={cn("h-7 w-[120px] shrink-0 flex items-center justify-between rounded-sm hover:bg-neutral-200/80 px-1.5 text-sm overflow-hidden")}>
                    <span className="truncate">
                        {
...
export const Toolbar = () => {
    ...
    return <div className="bg-[#F1F4F9] px-2.5 py-0.5 rounded-[24px] min-h-[40px] flex items-center gap-x-0.5 overflow-y-auto">
        {sections[0].map((item) => (
            <ToolbarButton key={item.label} {...item} />
        ))}
        <Separator orientation="vertical" className="h-6 bh-neutral-300" />
        <FontFamilyButton />
        <Separator orientation="vertical" className="h-6 bh-neutral-300" />
        <HeadingLevelButton />
        <Separator orientation="vertical" className="h-6 bh-neutral-300" />
